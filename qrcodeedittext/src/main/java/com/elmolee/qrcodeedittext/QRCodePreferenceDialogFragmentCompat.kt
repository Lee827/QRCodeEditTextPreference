...
class QRCodePreferenceDialogFragmentCompat : PreferenceDialogFragmentCompat() {
    ...
    override fun onBindDialogView(view: View) {
        ...
        view.imageButton_qrcode.setOnClickListener {
            when (Utils.getNumberOfCameras(context!!) > 0) {
                false -> Toast.makeText(context,  getString(R.string.toast_camera_error), Toast.LENGTH_SHORT).show()
                true -> {
                    val activityWeakRef = WeakReference(this)
                    activityWeakRef.get()?.apply {
                        var intent = Intent(context, QRCodeScannerActivity::class.java)
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        if (requestCode == 0) {
            if (resultCode == Activity.RESULT_OK) {
                ...
                val returnString = data?.getStringExtra(Intent.EXTRA_TEXT) ?: return
